...
def model_acf(xcor, acor, fcut, fmin=None, fmax=None, crit_fap=0.003, method='smooth', window_length=None):
    ...
    xf_L, yf_L, freqs_L, faps_L = FFT_estimator(xcor, acor_emp[0], fmin=fmin, fmax=fmax, crit_fap=crit_fap)
    ...
    xf_H, yf_H, freqs_H, faps_H = FFT_estimator(xcor, acor_emp[1], fmin=fmin, fmax=fmax, crit_fap=crit_fap)
...
def generate_synthetic_noise(xcor, acor, n, sigma):
    ...
    try:
        ...
        try:
            ...
            eps = np.min(np.abs(acor[acor != 0]))*1e-3
