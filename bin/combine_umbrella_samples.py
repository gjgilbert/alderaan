        ...
        with fits.open(f) as HDUList:
            ...
            for k in raw_data[ext].keys():
                if raw_data[ext][k].dtype == '>f8':
                    ...
                    warnings.warn("Expected all dtypes to be '>f8' or '>i8', but found '{0}'".format(raw_data[ext][k].dtype))
...
y, x, _ = ax.hist(g, bins=bins, density=True, histtype="step", color="C{0}".format(PLANET_NO), lw=2)
...
plt.savefig(FIGURE_DIR + TARGET + '_transition_gamma_{0}.png'.format(PLANET_NO), bbox_inches='tight')

if iplot: plt.show()
else: plt.close()
    
print("{0} out of {1} transition samples are consistent with a grazing transit".format(np.sum(g < 1), len(g)))
...
if len(psi_fxns) == 1:
    ...
    for i in range(3):
        ...
        uweights.append(1./wd)

    # concatenate samples and track weights for ease of use
    samples = pd.concat([raw_data['N'], raw_data['T'], raw_data['G']], ignore_index=True)[keys]
...
synth_samps = generate_synthetic_samples(samples.values, bandwidths.values, n_up=len(samples), weights=weights)
...
for k in synth_samps.keys():
    out += astropy.stats.sigma_clip(synth_samps[k], cenfunc='median', stdfunc='mad_std', sigma=10).mask
    
# Make corner plot
fig = corner.corner(synth_samps[~out], color='C{0}'.format(PLANET_NO))
plt.savefig(FIGURE_DIR + TARGET + '_corner_{0}.png'.format(PLANET_NO), bbox_inches='tight')
...
data_info = [
            ...
            ['LD_U1',    'dimensionless', '1st quadratic stellar limbdark coeff',  'Kipping (2013)'],
            ['LD_U2',    'dimensionless', '2nd quadratic stellar limbdark coeff',  'Kipping (2013)'],
]
...
hdu.header['DATETIME'] = (datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "Date and time of analysis")
hdu.header['COMMENT'] = "ALDERAAN single planet transit fit -- Combined Synthetic Samples"

f_name = PROJECT_DIR + 'Traces/{0}/{0}-{1}_CSS_indep.fits'.format(TARGET, str(PLANET_NO).zfill(2))
...
print("")
print("+"*shutil.get_terminal_size().columns)
print("Umbrella sample processing complete {0}".format(datetime.now().strftime("%d-%b-%Y at %H:%M:%S")))
