...
def main():
    ...
    mast_files = glob.glob(DATA_DIR + f'kplr{KIC:09d}*.fits')
    mast_files.sort()
    ...
    if USE_SC:
        for i, mf in enumerate(mast_files):
            with fits.open(mf) as hdu_list:
                if hdu_list[0].header['OBSMODE'] == 'short cadence':
    ...
    for i, mf in enumerate(mast_files):
        with fits.open(mf) as hdu_list:
            if (hdu_list[0].header['OBSMODE'] == 'long cadence') \
            and ~np.isin(hdu_list[0].header['QUARTER'], sc_quarters):
    ...
    print(f"Oversampling factor = {long_cadence_oversample}")
    ...
    holczer_file = os.path.join(PROJECT_DIR, 'Catalogs/holczer_2016_kepler_ttvs.txt')
    ...
    for npl in range(NPL):
        if np.isfinite(holczer_pers[npl]):
            ...
            yomc  = np.copy(holczer_tts[npl] - ephem)
    ...
    if MISSION == 'Simulated':
        ...
        for npl in range(NPL):
            ...
            tts_file = os.path.join(PROJECT_DIR, f'Simulations/{RUN_ID}/{TARGET}_{npl}.tts')
    ...
    if len(sc_flux) > 0: sc_flux_lin = np.hstack(sc_flux)
    else: sc_flux_lin = np.array([])

    if len(lc_flux) > 0: lc_flux_lin = np.hstack(lc_flux)
    else: lc_flux_lin = np.array([])
    ...
    with pm.Model() as shape_model:
        ...
        for j, q in enumerate(quarters):
            ...
            obs[j] = pm.Normal(f"obs_{j}", mu=model_flux[j], sd=flux_err[j], observed=all_flux[q])
    ...
    with pm.Model() as indep_model:
        ...
        for npl in range(NPL):
            ...
            tt_offset.append(pm.Normal(f"tt_offset_{npl}", mu=0, sd=1, shape=np.sum(use)))

            map_tts.append(pm.Deterministic(f"tts_{npl}",
        ...
        for j, q in enumerate(map_quarters):
            ...
            obs[j] = pm.Normal(f"obs_{j}", mu=model_flux[j], sd=flux_err[j], observed=map_flux[q])
    ...
    for npl in range(NPL):
        ...
        if omc_freqs[npl] is not None:
            ...
            plt.plot(xtime, LS.model(xtime, omc_freqs[npl])*24*60, c=f"C{npl}", lw=3)
            if IPLOT:
                plt.show()
            else:
                plt.close()
    ...
    for npl, p in enumerate(planets):
        ...
        plt.text(xtime.min(), yomc.min()*24*60, f"measured error = {err_:.1f} min", fontsize=16, ha='left', backgroundcolor='w')
        plt.text(xtime.max(), yomc.min()*24*60, f"residual RMS = {rms_:.1f} min", fontsize=16, ha='right', backgroundcolor='w')
        plt.savefig(os.path.join(FIGURE_DIR, TARGET + f'_ttvs_quick_{npl:02d}.png'), bbox_inches='tight')
        if IPLOT:
            plt.show()
        else:
            plt.close()
    ...
    for npl, p in enumerate(planets):
        ...
        if len(tts) == 0:
            ...
            plt.plot(t_binned*24, f_binned, 'o', ms=8, color=f'C{npl}', label=f"{TARGET}-{npl}")
            ...
            plt.savefig(os.path.join(FIGURE_DIR, TARGET + f'_folded_transit_{npl:02d}.png'), bbox_inches='tight')
            if IPLOT:
                plt.show()
            else:
                plt.close()
    ...
    print("")
    print("+"*shutil.get_terminal_size().columns)
    print(f"Automated lightcurve detrending complete {datetime.now().strftime('%d-%b-%Y at %H:%M:%S')}")
