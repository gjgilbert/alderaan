...
def main():
    ...
    if MISSION == "Kepler":
        ...
        target_dict = pd.read_csv(
            os.path.join(PROJECT_DIR, f"Simulations/{RUN_ID}/{RUN_ID}.csv")
        )
    else:
    ...
    for npl in range(NPL):
        ...
        ttv_scatter[npl] = astropy.stats.mad_std(
            indep_transit_times[npl] - quick_transit_times[npl]
        )
    ...
    if sc is not None:
        ...
        for npl in range(NPL):
            ...
            sc_wide_mask[npl] = make_transit_mask(sc.time, tts, wide_size)
            sc_narrow_mask[npl] = make_transit_mask(sc.time, tts, narrow_size)
            sc_regular_mask[npl] = make_transit_mask(sc.time, tts, regular_size)
    ...
    if lc is not None:
        ...
        for npl in range(NPL):
            ...
            lc_wide_mask[npl] = make_transit_mask(lc.time, tts, wide_size)
            lc_narrow_mask[npl] = make_transit_mask(lc.time, tts, narrow_size)
            lc_regular_mask[npl] = make_transit_mask(lc.time, tts, regular_size)
