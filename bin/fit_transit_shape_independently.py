...
for npl in range(NPL):
    if npl == PLANET_NO:
        ...
        with pm.Model() as shape_model:
            ...
            if UMBRELLA == 'N':
                # draw (log_r, b) --> (r,b)
                log_r = pm.Uniform('LN_ROR', lower=np.log(rmin), upper=np.log(rmax), testval=np.log(np.sqrt(DEPTHS[npl])))
                ...
                log_lam = pm.Uniform('LN_LAM', lower=np.log((g+1)*rmin**2), upper=np.log((g+1)*rmax**2))
            ...
            log_dur = pm.Uniform('LN_DUR14', lower=np.log(scit), upper=np.log(3*DURS[npl]), testval=np.log(DURS[npl]))
            dur = pm.Deterministic('DUR14', T.exp(log_dur))

            # polynomial TTV parameters (coefficients for Legendre polynomials)
            C0 = pm.Normal('C0', mu=0, sd=3*np.std(quick_transit_times[npl] - ephemeris[npl]))
            C1 = pm.Normal('C1', mu=0, sd=3*np.std(quick_transit_times[npl] - ephemeris[npl]))
            
            # transit times
            transit_times = pm.Deterministic('TTIMES', quick_transit_times[npl] + C0*Leg0[npl] + C1*Leg1[npl])
            ...
            log_jit = pm.Normal('LN_JIT', mu=np.log(vbq/10), sd=5.0*np.ones(nq), shape=nq)
...
data_info = [
            ...
            ['LN_ROR',   'dimensionless', 'Natural log of planet-to-star radius ratio',       'Uniform'],
            ...
            ['LD_U1',    'dimensionless', '1st quadratic stellar limb darkening coefficient', 'Kipping (2013)'],
            ['LD_U2',    'dimensionless', '2nd quadratic stellar limb darkening coefficient', 'Kipping (2013)'],
...
for k in list(trace.posterior.keys()):
    if k == 'LD_U':
        ...
        for j, q in enumerate(which_quarters):
            trace_df['FLUXZPT_{0}'.format(str(q).zfill(2))] = list(trace.posterior['FLUXZPT'][:,:,j].data.reshape(-1))

    elif k == 'LN_JIT':
        for j, q in enumerate(which_quarters):
            trace_df['LN_JIT_{0}'.format(str(q).zfill(2))] = list(trace.posterior['LN_JIT'][:,:,j].data.reshape(-1))
...
hdu.header['DATETIME'] = (timestamp.strftime('%Y-%m-%d %H:%M:%S'), "Date and time of analysis")
hdu.header['COMMENT'] = "ALDERAAN single planet transit fit"

f_name = PROJECT_DIR + 'Traces/{0}/{0}-{1}_{2}_indep.fits'.format(TARGET, str(PLANET_NO).zfill(2), UMBRELLA)
...
with fits.open(f_name) as hduL:
    ...
    data_head['RUNTIME'] = (np.round(trace.posterior.attrs['sampling_time'],2), "Duration of model sampling (seconds)")
