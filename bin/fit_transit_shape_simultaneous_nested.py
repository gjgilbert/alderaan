...
def main():
    ...
    if os.path.exists(os.path.join(RESULTS_DIR , f'{TARGET}_lc_filtered.fits')):
        lc = io.load_detrended_lightcurve(os.path.join(RESULTS_DIR, f'{TARGET}_lc_filtered.fits'))
        lc.season = lc.quarter % 4
    else:
        lc = None

    if os.path.exists(os.path.join(RESULTS_DIR , f'{TARGET}_sc_filtered.fits')):
        sc = io.load_detrended_lightcurve(os.path.join(RESULTS_DIR, f'{TARGET}_sc_filtered.fits'))
    ...
    hdu_list = io.to_fits(results, PROJECT_DIR, RUN_ID, TARGET, NPL)
    ...
    print("")
    print("+"*shutil.get_terminal_size().columns)
    print(f"Exoplanet recovery complete {datetime.now().strftime('%d-%b-%Y at %H:%M:%S')}")
