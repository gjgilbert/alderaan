...
def main():
    ...
    if MISSION == "Kepler":
        ...
        target_dict = pd.read_csv(PROJECT_DIR + f"Simulations/{RUN_ID}/{RUN_ID}.csv")
    else:
    ...
    if all(u == U1[0] for u in U1):
        U1 = U1[0]
    else:
        raise ValueError("There are inconsistencies with U1 in the csv input file")

    if all(u == U2[0] for u in U2):
        U2 = U2[0]
    else:
        raise ValueError("There are inconsistencies with U2 in the csv input file")

    if np.any(np.isnan(PERIODS)):
        raise ValueError("NaN values found in input catalog")
    if np.any(np.isnan(DEPTHS)):
        raise ValueError("NaN values found in input catalog")
    if np.any(np.isnan(DURS)):
        raise ValueError("NaN values found in input catalog")
    ...
    if sc is not None:
        ...
        for npl in range(NPL):
            sc_mask[npl] = make_transit_mask(
                ...
                masksize=np.max([1 / 24, 1.5 * DURS[npl]]),
            )
    ...
    for j, q in enumerate(quarters):
        if all_dtype[q] == "short":
            ...
            texp_offsets[q] = np.linspace(
                -exptime[q] / 2.0, exptime[q] / 2.0, oversample[q]
            )
